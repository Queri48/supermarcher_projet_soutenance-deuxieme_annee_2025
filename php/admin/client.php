...
$stmt = $conn->prepare("SELECT id, nom, prenom, email, tel, adresse, role, datetime FROM utilisateur");
$stmt->execute();
                ...
                <?php foreach ($utilisateurs as $index => $user): ?>
                    <tr>
                        ...
                        <td class="text-center">
                            <a href="modifier_utilisateur.php?id=<?= $user['id'] ?>" class="btn btn-sm btn-outline-success me-1">
                                <i class="fas fa-edit"></i>
                            </a>
                            <a href="supprimer_utilisateur.php?id=<?= $user['id'] ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet utilisateur ?');">
